...
export class ChecklistEffect {
  ...
  //           this.store.dispatch(removeTermFormChecklistAction({ ids: deleted.map(t => t.checklist_term.id) }));
  //         }),
  ...
  //   })
  // ));
  // addItemToChecklist$ = createEffect(() => this.actions$.pipe(
  //   ofType(addItemToChecklist),
  ...
  //         this.store.dispatch(addItemToChecklistTermsAction({ ids: [item.checklist_term.id] }))
  //       }),
